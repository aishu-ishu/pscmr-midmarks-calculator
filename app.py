...
MAX_CONCURRENT_JOBS = 2  # Match this to your Gunicorn worker count
active_jobs = 0
...
def process_image():
    ...
    # Safely check and claim an active slot
    with job_lock:
        if active_jobs >= MAX_CONCURRENT_JOBS:
            return jsonify({
                "error": "Server is currently busy processing other mark sheets. Please try again in a few minutes."
            }), 503
        active_jobs += 1
    ...
    try:
        ...
        # Always free up the slot when done, even if errors occur
        with job_lock:
            active_jobs -= 1
